...
def informType(df):
    ...
    try:
        ...
        df=changeType(df, "Codigo_Setor_Censitario", 1)
        df=changeType(df, "Codigo_Localidade", 1)
        df=changeType(df, "Codigo_Municipio", 1)
        df=changeType(df, "Area_km2", 2)
        df=changeType(df, "Domicilios", 1)
        df=changeType(df, "Moradores", 1)
        df=changeType(df, "Percentual_Cobertura", 2)
