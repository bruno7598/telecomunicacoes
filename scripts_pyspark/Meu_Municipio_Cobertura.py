...
def file(file="Meu_Municipio_Cobertura.csv"):
    try:    
...
def readFormatCsv():
    try:
        ...
        ses = session()
        df = ses.read.csv(f"{file()}", header=True, sep = ";", inferSchema=True, encoding='UTF-8')
        return df
    except Exception as e:
...
if __name__=="__main__":
    ...
    try: 
        ...
        df=changeType(df, 'Moradores_Cobertos', 1)
        df=changeType(df, 'Domicilios_Cobertos', 1)
        df=changeType(df, 'Area_km2_Coberta', 2)
        df=changeType(df, 'Area_Municipio_km2', 2)
